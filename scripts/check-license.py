#!/usr/bin/env python3
"""Pin the unmodified Apache License 2.0 text shipped by this repository."""
...
def check(contents: str) -> None:
    if not contents.startswith(HEADER):
        raise ValueError("Apache-2.0 header mismatch")
    if sha256(contents.encode()).hexdigest() != TEXT_SHA256:
...
if __name__ == "__main__":
    try:
        ...
        if sys.argv[1:] == ["--self-check"]:
            for changed in ("SPDX-License-Identifier: Apache-2.0\n" + original,
                            original.replace("Grant of Patent License", "Grant of Patent Licence", 1)):
                try:
                    check(changed)
                except ValueError:
                    continue
            ...
            print("license: unmodified Apache-2.0 text matches")
